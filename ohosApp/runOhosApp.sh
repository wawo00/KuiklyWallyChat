...
if [ -z "$targets" ]; then
  ...
  $HDC_BIN -t "$target_id" shell aa force-stop com.wally.demo.kuiklywallychat
  $HDC_BIN -t "$target_id" install entry/build/default/outputs/default/entry-default-signed.hap
  $HDC_BIN -t "$target_id" shell aa start -a EntryAbility -b com.wally.demo.kuiklywallychat
  done
fi